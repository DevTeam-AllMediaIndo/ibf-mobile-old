    ...
    if(isset($_POST['type'])){
        ...
        if($type == 'demo'){
            ...
            if(mysqli_num_rows($SQL_QUERY) < 1) {
                ...
                $createaccount = MetaCreateDemo('fullname='.str_replace(" ", "_", $user1['MBR_NAME']).'&group=demoIBF-Forex&master_pass='.$meta_pass.'&investor_pass='.$meta_investor.'&email='.$user1['MBR_EMAIL'].'&comment=metaapi1&leverage=100&login=0');
                // print_r($createaccount['message']);
                // $createaccount1 = mt4api_demo_post(41, "AccountCreate41", 'name='.str_replace(" ", "_", $user1['MBR_NAME']).'&group=demoIBF-Forex&password='.$meta_pass.'&investor='.$meta_investor.'&country='.$user1['MBR_COUNTRY'].'&state=&email='.$user1['MBR_EMAIL'].'&city='.$user1['MBR_CITY'].'&comment=metaapi&leverage=100&zip_code=0&phone='.$user1['MBR_PHONE'].'&address='.$user1['MBR_ADDRESS'].'&enable=true&enable_change_password=true&phone_password='.$meta_phone.'&login=0');
                $createaccount = ($createaccount['message']);
                if($createaccount['Login']){
                    $result = MetaDepositDemo('login='.($createaccount['Login']).'&amount=10000&comment=mobile_deposit1');
                    mysqli_query($db, '
...
?>
        ...
        <div class="alert me-3 ms-3 rounded-s bg-green-dark" role="alert">
            ...
            <strong class="alert-icon-text">Account demo has been created <?= $msg ?>.</strong>
...
<?php
            ...
            if(mysqli_num_rows($SQL_QUERY) < 1) {
                ...
                insert_log($user1['MBR_ID'], 'Memulai Pembuatan Akun Real Baru');
            ...
            if(isset($_GET['type'])){
                ...
                if($type == 'demo'){
                    ...
                    if(mysqli_num_rows($SQL_QUERY) < 1) {
                        ...
                        $createaccount = MetaCreateDemo('fullname='.str_replace(" ", "_", $user1['MBR_NAME']).'&group=demoIBF-Forex&master_pass='.$meta_pass.'&investor_pass='.$meta_investor.'&email='.$user1['MBR_EMAIL'].'&comment=metaapi2&leverage=100&login=0');
                        // $createaccount = mt4api_demo_post(41, "AccountCreate41", 'name='.str_replace(" ", "_", $user1['MBR_NAME']).'&group=demoIBF-Forex&password='.$meta_pass.'&investor='.$meta_investor.'&country='.$user1['MBR_COUNTRY'].'&state=&email='.$user1['MBR_EMAIL'].'&city='.$user1['MBR_CITY'].'&comment=metaapi&leverage=100&zip_code=0&phone='.$user1['MBR_PHONE'].'&address='.str_replace(' ', '_', $user1['MBR_ADDRESS']).'&enable=true&enable_change_password=true&phone_password='.$meta_phone.'&login=0');
                        // echo '<pre>';
                        // var_dump($createaccount, $user1['MBR_ID'], 'test');
                        // echo '</pre>';
                        // die;
                        $createaccount = ($createaccount['message']);
                        $result = MetaDepositDemo('login='.($createaccount['Login']).'&amount=10000&comment=mobile_deposit');
                        if($createaccount['Login']){
                            mysqli_query($db, '
                                INSERT INTO tb_racc SET
                                tb_racc.ACC_MBR = '.$user1['MBR_ID'].',
...
?>
    ...
    <div class="alert me-3 ms-3 rounded-s bg-green-dark" role="alert">
        <span class="alert-icon"><i class="fa fa-check font-18"></i></span>
        <h4 class="text-uppercase color-white">Success</h4>
        <strong class="alert-icon-text">Account demo has been created 2.</strong>
        <button type="button" class="close color-white opacity-60 font-16" data-bs-dismiss="alert" aria-label="Close">&times;</button>
    </div>
