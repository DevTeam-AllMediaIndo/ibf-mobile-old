<?php
    if($_SERVER['REQUEST_METHOD'] === 'POST') {
        if(isset($_POST['submit'])){
            $SQL_CHECK_SCHDL = mysqli_query($db, '
                SELECT
                    tb_schedule.ID_SCHD
                FROM tb_schedule
                WHERE tb_schedule.SCHD_ID = '.$user1["MBR_ID"].'
                AND (tb_schedule.SCHD_STS = -1 OR tb_schedule.SCHD_STS = 0)
            ');
            if(mysqli_num_rows($SQL_CHECK_SCHDL) < 1){
                if(isset($_POST['sesi']) &&
                isset($_POST['alamat']) &&
                ...
                isset($_POST['demoacc'])){
                    ...
                    logerr("Parameter Ke-1 Tidak Lengkap", "Jadwal Temu", $user1["MBR_ID"]);
                    die("<script>location.href = 'home.php?page=dashboard&notif=".base64_encode('Parameter tidak Lengkap')."'</script>");
                }
            }else{ 
                logerr("Sudah Melakukan Request Jadwal Temu", "Jadwal Temu", $user1["MBR_ID"]);
                die("<script>location.href = 'home.php?page=dashboard&notif=".base64_encode('Anda sudah mengirim jadwal temu.')."'</script>"); 
            };
        }; 
    };

    // if(isset($_GET["testtele"])){
    //     $mesg = 'Test Notif';
                    ...
                    <form method="POST" id="frm">
                        <div class="form-group boxed">
                            ...
                            <div class="col">
                                ...
                                <button class="btn btn-primary rounded-pill btn-login w-100 mb-2" id="btn-next" type="submit" name="submit2">Submit</button>
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    function GetSelectedTextValue2(idt_type) {
        ...
        if(selVal == 'Passport'){
            min_length  = 8;
            max_length  = 16;
            message     = 'No Identitas harus lebih dari '+min_length+' digit dan kurang dari atau sama dengan '+max_length+' digit.';
            ...
            min_length = 15;
            max_length = 16;
            message     = 'No Identitas harus '+max_length+' digit';
            console.log('KTP')
        ...
        no_idt.addEventListener('keyup', function(evt) {
            if(no_idt.value.length < min_length || no_idt.value.length > max_length) {
