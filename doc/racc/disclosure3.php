    ...
    if(isset($_POST['aggree'])){
        if(isset($_GET['id'])){
            ...
            if($aggree == 'Yes'){
                ...
                die ("<script>location.href = 'home.php?page=racc/aplikasipembukaanrekening01&id=".$id_live."'</script>");
            }else{die("<script>location.href = 'home.php?page=racc/disclosure&id=".$id_live."'</script>");};
        } else {
            logerr("Parameter Ke-2 Tidak Lengkap", "Disclosure 3", $user1["MBR_ID"]);
            die ("<script>alert('please try again');location.href = 'home.php?page=racc/disclosure&id=".$id_live."'</script>"); 
        };
    } else {
        logerr("Parameter Ke-1 Tidak Lengkap", "Disclosure 3", $user1["MBR_ID"]);
        die ("<script>alert('please try again');location.href = 'home.php?page=racc/disclosure&id=".$id_live."'</script>"); 
    };
};
?>
