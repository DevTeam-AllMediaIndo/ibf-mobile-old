<?php
    ...
    if(mysqli_num_rows($SQL_QUERYACC) > 0) {
        ...
        $R_INVSTG = $RESULT_QUERYACC['ACC_F_APP_PAILIT'];
    } else {
        $R_INVSTG = '';
    }
        ...
        if(isset($_POST['exp_level'])){
            ...
            if($exp_level == 'Ya'){
                ...
                $EXEC_SQL = mysqli_query($db,'
                    UPDATE tb_racc SET
                        tb_racc.ACC_F_APP_PAILIT   = "'.$exp_level.'"
                    WHERE MD5(MD5(tb_racc.ID_ACC)) = "'.$id_live.'"
                ') or die (mysqli_error($db));
                die ("<script>location.href = 'home.php?page=racc/aplikasipembukaanrekening06&id=".$id_live."'</script>");
            };
        } else { 
            logerr("Parameter Ke-1 Tidak Lengkap", "Aplikasi Pembukaan Rekening Hal-5 (REGOL)", $user1["MBR_ID"]);
...
<form method="post">
    <div class="card card-style">
        <div class="content">
            ...
            <div class="text-center"><strong>Apakah Anda telah dinyatakan pailit oleh Pengadilan?</strong></div>
        </div>
    </div>
    <div class="row">
        <div class="col-6">
                ...
                <div class="card-body" style="font-size : 15px; ">
                    <div class="form-check form-check-inline">
                        <input class="form-check-input radio_css" type="radio"  id="radioInlineDefault1" 
                            <?php if($R_INVSTG == 'Ya' || $RESULT_DT["ACC_F_APP_PAILIT"] == 'Ya'){ echo 'checked'; } ?> value="Ya" name="exp_level" required
                        >
                ...
                <div class="card-body" style="font-size : 15px; ">
                    <div class="form-check form-check-inline">
                        <input class="form-check-input radio_css" type="radio"  id="radioInlineDefault2" 
                            <?php if($R_INVSTG == 'Tidak' || $RESULT_DT["ACC_F_APP_PAILIT"] == 'Tidak'){ echo 'checked'; } ?> value="Tidak" name="exp_level"
                        >
