    ...
    if(isset($_POST['type'])){
        ...
        if($type == 'demo'){
            ...
            if(mysqli_num_rows($SQL_QUERY) < 1) {
                ...
                // $createaccount = mt4api_connect_post('account', 'name='.$user1['MBR_NAME'].'&group=demoIBF-Forex&password='.$meta_pass.'&investor='.$meta_investor.'&country='.$user1['MBR_COUNTRY'].'&state=&email='.$user1['MBR_EMAIL'].'&city='.$user1['MBR_CITY'].'&comment=metaapi&leverage=100&zip_code=0&phone='.$user1['MBR_PHONE'].'&address='.$user1['MBR_ADDRESS'].'&enable=1&enable_change_password=1&phone_password='.$meta_phone.'&login=0');
                // $result = mt4api_connect_post('deposit/'.($createaccount['login']), 'amount=10000&comment=mobile deposit');

                $createaccount = mt4api_demo_post(41, "AccountCreate4", 'name='.$user1['MBR_NAME'].'&group=demoIBF-Forex&password='.$meta_pass.'&investor='.$meta_investor.'&country='.$user1['MBR_COUNTRY'].'&state=&email='.$user1['MBR_EMAIL'].'&city='.$user1['MBR_CITY'].'&comment=metaapi&leverage=100&zip_code=0&phone='.$user1['MBR_PHONE'].'&address='.$user1['MBR_ADDRESS'].'&enable=true&enable_change_password=true&phone_password='.$meta_phone.'&login=0');
