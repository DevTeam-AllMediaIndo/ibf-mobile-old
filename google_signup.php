...
if(isset($_GET["code"])){
    ...
    if(!isset($token['error'])){
        ...
        $SQL_QUERY = mysqli_query($db, 'SELECT MBR_ID FROM tb_member WHERE LOWER(MBR_EMAIL) = LOWER("'.$email.'") LIMIT 1');
        if($SQL_QUERY && mysqli_num_rows($SQL_QUERY) > 0){
            $sql_rsl1 = mysqli_fetch_assoc($SQL_QUERY);
            logerr("Email: $email sudah terdaftar", "G-sigunp" , $sql_rsl1["MBR_ID"]);
            die ("<script>alert('email already registered, please login with your email');location.href = './'</script>");
        } else {
            $MBR_PASS = $id;
            $MBR_KODE = 'admin';
